...
def HMM2_viterbi(emission_dict, transition_dict, observations, sentence):
  ...
  for state_w in states:
    ...
    if ((obs in emission_dict[state_w]) or (obs == TOKEN)):
      emis_frac = emission_para_token(emission_dict, obs, state_w, k=8)
...
if __name__ == '__main__':
    ...
    for i in design_dataset:
        ...
        evaluation = root_dir + "{folder}/test.in".format(folder = i)
        ...
        # create output file
        with open(root_dir + "{folder}/test.p5.k8.out".format(folder = i), "w") as g:
